...
void yawRateController()
{
	...
	// PI controller in velocity form
...
void torqueVectoring()
{
	...
	// Report the saturated value back to the yaw rate PI as the previous output, which should prevent integrator windup since it's in velocity form
	saturatedDM = (motorTorqueLeftRef - motorTorqueRightRef) / 2; // Again, flip the signs here if using CCW yaw
...
void calculateMotorTorqueRef()
{
	...
	*/
	...
	float torqueCM = accInput * MaxCMTorqueFraction * fmax(maxMotorTorqueLeft, maxMotorTorqueRight);
